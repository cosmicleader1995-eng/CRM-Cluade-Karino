...
function mergeServerDBs(local: DatabaseSchema, remote: DatabaseSchema): DatabaseSchema {
  ...
  // Merge archives uniquely by dateShamsi (1 archive file per calendar day)
  ...
  allArchives.forEach((a: any) => {
    ...
    const existing = archiveMap.get(dateKey);
    ...
    if (!existing || aCount > exCount || (aCount === exCount && new Date(a.timestamp || 0) > new Date(existing.timestamp || 0))) {
      archiveMap.set(dateKey, {
        ...a,
        id: `arch-${dateKey.replace(/\//g, '')}`,
        dateShamsi: dateKey
      });
    }
  });
  const mergedArchives = Array.from(archiveMap.values())
    .filter((a: any) => (Array.isArray(a.reports) && a.reports.length > 0) || !a.autoGenerated)
...
app.post('/api/db/archive', async (req, res) => {
  ...
  archive.id = `arch-${dateKey.replace(/\//g, '')}`;
  archive.dateShamsi = dateKey;

  const existingIdx = db.archives.findIndex((a: any) => String(a.dateShamsi).trim() === dateKey);
  if (existingIdx >= 0) {
  ...
  db.archives.forEach((a: any) => {
    if (!a || !a.dateShamsi) return;
    const k = String(a.dateShamsi).trim();
    map.set(k, a);
  });
  ...
  addAuditLog(db, {
    ...
    category: 'SYSTEM',
    level: 'INFO',
    message: `رکورد بایگانی مکانیزه «${archive.fileName}» ذخیره و به‌روزرسانی شد.`
  });
