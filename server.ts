...
async function runServerNightlyArchiver() {
  try {
    ...
    // 1. Daily Calls & Follow-ups Archive
    ...
    // 2. Daily Periodic Overall Reports Archive
      ...
      if (weekPeriodic.length > 0) {
        ...
        if (!exists || (exists.overallReports || []).length !== weekPeriodic.length) {
          const newArch = {
            ...
            periodTitle: 'بایگانی هفتگی گزارشات مشاورین (پنج‌شنبه)',
    ...
    // 4. Monthly Reports Archive (Last Working Day of Shamsi Month at 23:00)
      ...
      if (monthPeriodic.length > 0) {
        ...
        if (!exists || (exists.overallReports || []).length !== monthPeriodic.length) {
          const newArch = {
            ...
            periodTitle: 'بایگانی ماهانه گزارشات استراتژیک (پایان ماه)',
